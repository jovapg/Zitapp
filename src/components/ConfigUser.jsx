...
export default function ConfigUser({ show, handleClose }) {
  ...
  useEffect(() => {
    if (show && userId) {
      axios.get(`http://localhost:8081/api/users/${userId}`)
        .then((res) => {
          // *** CAMBIO CLAVE AQUÍ: DESESTRUCTURAR 'imagenPerfil' EN LUGAR DE 'imagenUrl' ***
          ...
          setFormData({
            ...
            telefono: telefono || '',
            contrasena: '', 
            // *** Y ASIGNAR 'imagenPerfil' A 'imagenUrl' EN EL ESTADO LOCAL ***
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // *** CAMBIO ADICIONAL: ASEGURAR QUE SE ENVÍA 'imagenPerfil' AL BACKEND ***
      // Si el campo en el backend se llama 'imagenPerfil', debes enviarlo así
      if (datosAEnviar.imagenUrl === '') {
        datosAEnviar.imagenUrl = null; 
      }
      // Esto es crucial si tu API PUT espera 'imagenPerfil' y no 'imagenUrl'
        ...
        <Row className="g-0">
          ...
          <Col md={5} className="d-flex flex-column align-items-center p-4" style={{
            ...
            <div className="position-relative mb-3">
              <Image 
                src={formData.imagenUrl || "https://via.placeholder.com/150x150?text=Agregar+Foto"} 
                roundedCircle 
